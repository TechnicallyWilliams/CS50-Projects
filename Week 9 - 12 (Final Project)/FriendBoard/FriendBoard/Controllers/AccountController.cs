...
using FriendBoard.Providers;
using FriendBoard.Results;
...
namespace FriendBoard.Controllers
{
    ...
    public class AccountController : ApiController
    {
        ...
        public async Task<IHttpActionResult> Register(RegisterViewModel data)
        {
            ...
            //bool status = !db.Users.Any(u => u.UserName == data.UserName && u.Email == data.Email);
            ApplicationUser User = new ApplicationUser() { DateCreated = DateTime.Now, DateUpdated = DateTime.Now, IsActive = true, TimesLoggedIn = 1, UserName = data.UserName, Email = data.Email };
            IdentityResult result = await UserManager.CreateAsync(User, data.Password); //replace User with
            ...
            if (result.Succeeded)
            {
                ...
                await UserManager.GenerateUserTokenAsync("", UserId);
                //UserManager.GenerateUserTokenAsync("", UserId);
